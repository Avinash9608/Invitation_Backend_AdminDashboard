...
const WeddingForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Redirect to the link after successful submission
        window.location.href =
          "https://invitation-card-weddingpage.vercel.app/";
      } else {
        const errorData = await response.json();
        console.error("Error submitting form:", errorData);
        alert(
  ...
  return (
    <div className="Wedding_Form_Sec">
      <div className="WeddingFormMid mx-auto  rounded-lg overflow-hidden shadow-2xl transform hover:scale-105 transition-transform duration-300">
        <h2 className="text-4xl mb-1 text-center">Wedding Template Form</h2>
        <form onSubmit={handleSubmit}>
          {/* Basic Info Fields */}
          {step === 1 && (
            <>
          ...
          {step === 5 && (
            <>
              ...
              <button
                ...
                className="py-3 w-24 bg-blue-500 text-white font-semibold rounded-md shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                onClick={async (event) => {
                  await handleSubmit(event); // Call handleSubmit and wait for completion
                }}
              >
                Submit
              </button>
